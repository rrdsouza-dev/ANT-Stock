-- Sincroniza perfis permitidos e garante compatibilidade completa com recuperacao de senha.
...
do $$
begin
    if exists (select 1 from pg_type where typname = 'perfil_codigo') then
        if exists (
            select 1
            from pg_enum e
