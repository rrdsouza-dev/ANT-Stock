...
class RepositorioItemPedido(RepositorioSQL[ItemPedido]):
    ...
    async def por_pedido(self, pedido_id: UUID) -> list[ItemPedido]:
        resultado = await self.sessao.execute(select(ItemPedido).where(col(ItemPedido.pedido_id) == pedido_id))
