...
async def listar(
    ...
    itens = await ServicoEstoque(sessao).listar_categorias(usuario, deposito_id, inicio=inicio, limite=limite)
...
async def buscar(
    ...
    return CategoriaSaida.model_validate(
        await ServicoEstoque(sessao).buscar_categoria(usuario, deposito_id, categoria_id)
    )
...
async def criar(
    ...
    usuario, deposito_id = usuario_deposito
    item = await ServicoEstoque(sessao).criar_categoria(usuario, deposito_id, dados.model_dump())
...
async def remover(
    ...
    usuario, deposito_id = usuario_deposito
    await ServicoEstoque(sessao).remover_categoria(usuario, deposito_id, categoria_id)
